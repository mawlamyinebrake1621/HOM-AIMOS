...
test('batch verification admits self-signed Genesis events before operator master creation', () => {
  const source = fs.readFileSync(path.join(ROOT, 'services/observe/event-ledger.js'), 'utf8');
  assert.match(source, /LEFT JOIN aimos_master_identity master\s+ON master\.id = 1/);
  assert.doesNotMatch(source, /CROSS JOIN aimos_master_identity master/);
  assert.match(source, /certBody\.issuer === row\.signer_agent_id\s+\? row\.pubkey\s+: row\.master_pubkey/);
  assert.match(source, /if \(!certAuthority\) throw new Error\('event_ledger_master_identity_missing'\)/);
});
