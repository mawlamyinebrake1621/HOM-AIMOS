...
function verifyManuscriptBoundary(tex, pdfText, contract, claims) {
  ...
  assert(normalizedTex.includes('baseline-aware attack-attributable result'));
  assert(pdfText.includes('1/98 (1.02%)'));
...
function verifyFirstPage(tex, contract) {
  const requirements = [
    ...
    'publication-authorized for arXiv submission',
    'arXiv identifier',
  ];
...
export function verifyMutMemV2Manuscript({ verifyRetainedAudit = true } = {}) {
  ...
  const unsigned = {
    ...
    publication_release_bound: true,
    ...
    provider_call: false,
    next_required_gate: 'ARXIV_SUBMISSION_AND_IDENTIFIER_RECORDING',
  };
  const result = {
    ...unsigned,
