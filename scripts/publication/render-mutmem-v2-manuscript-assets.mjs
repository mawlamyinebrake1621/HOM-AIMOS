...
function buildValues(contract, tables, claims, evidenceManifest, independent) {
  ...
  const poisonClean = rowById(historical, 'v1-poisonedrag-clean-asr');
  const poisonAttack = rowById(historical, 'v1-poisonedrag-attacked-asr');
  const poisonInduced = rowById(historical, 'v1-poisonedrag-induced-asr');
  ...
  const lines = [
    ...
    macro('MutMemReleaseURL', `\\url{${contract.release_binding.release_url}}`),
    macro('MutMemReleaseTag', texEscape(contract.release_binding.immutable_tag)),
    macro('MutMemReleaseCommit', `\\nolinkurl{${contract.release_binding.release_commit}}`),
    macro('MutMemSourceManifestSHA', `\\nolinkurl{${contract.release_binding.source_manifest_sha256}}`),
    macro('MutMemSourceManifestRoot', `\\nolinkurl{${contract.release_binding.source_files_root_sha256}}`),
    macro('MutMemSourcePackageSHA', `\\nolinkurl{${contract.release_binding.source_package_sha256}}`),
    macro('MutMemSBOMSHA', `\\nolinkurl{${contract.release_binding.sbom_sha256}}`),
    macro('MutMemChecksumsSHA', `\\nolinkurl{${contract.release_binding.checksums_sha256}}`),
    macro('MutMemReleaseCeremonyURL', `\\url{${contract.release_binding.release_ceremony_url}}`),
    ...
    macro('MutMemPoisonCleanAsrCount', poisonClean.count),
    macro('MutMemPoisonCleanAsrTotal', poisonClean.total),
    macro('MutMemPoisonAttackedAsrCount', poisonAttack.count),
    macro('MutMemPoisonAttackedAsrTotal', poisonAttack.total),
    macro('MutMemPoisonInducedCount', poisonInduced.count),
    macro('MutMemPoisonInducedTotal', poisonInduced.total),
    macro('MutMemPoisonInducedPercent', pct(poisonInduced.value, 2)),
...
PoisonedRAG adaptation, induced ASR & \MutMemPoisonInducedCount/\MutMemPoisonInducedTotal\ (\MutMemPoisonInducedPercent\%) & Baseline-aware effect: clean 2/100; attacked 3/100 \\
